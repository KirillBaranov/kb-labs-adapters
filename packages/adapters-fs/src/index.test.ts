...
describe('FilesystemStorageAdapter', () => {
  ...
  describe('Security', () => {
    it('should prevent directory traversal attacks', async () => {
      await expect(storage.write('../outside.txt', Buffer.from('bad'))).rejects.toThrow(
        'Path traversal detected'
      );
    });

    it('should prevent absolute path escaping baseDir', async () => {
      await expect(storage.write('/etc/passwd', Buffer.from('bad'))).rejects.toThrow(
        'Path traversal detected'
      );
    });
  });
  ...
  describe('Extended Methods - stat()', () => {
    ...
    it('should detect content types correctly', async () => {
      ...
      expect((await storage.stat('test.json'))?.contentType).toBe('application/json');
      expect((await storage.stat('test.md'))?.contentType).toBe('text/markdown');
